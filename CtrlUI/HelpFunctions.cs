        ...
        void LoadHelp()
        {
            try
            {
                if (sp_Help.Children.Count == 0)
                {
                    ...
                    sp_Help.Children.Add(new TextBlock() { Text = "You can use the included DirectXInput tool that will convert your DirectInput controller in to a Xbox controller so it can be used with almost every application available.", Style = (Style)Application.Current.Resources["TextBlockGray"], FontSize = (double)Application.Current.Resources["TextSizeMedium"], TextWrapping = TextWrapping.Wrap });
