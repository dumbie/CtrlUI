    ...
    partial class WindowMain
    {
        ...
        async Task HideProcessWindow(string processName, IntPtr windowHandleTarget, bool hideDelay, bool skipNotification)
        {
            try
            {
                ...
                if (!skipNotification)
                {
                    await Notification_Show_Status("AppMinimize", "Hiding " + processName);
                }
                ...
                if (!windowHidden)
                {
                    await Notification_Show_Status("Close", "Failed hiding application");
        ...
        async Task HideAllProcessWindows(string processName, List<IntPtr> windowHandleTargets, bool hideDelay, bool skipNotification)
        {
            try
            {
                ...
                if (!skipNotification)
                {
                    await Notification_Show_Status("AppMinimize", "Hiding all " + processName);
                }
