        ...
        async Task ShowProcessWindowAuto(DataBindApp dataBindApp, ProcessMulti processMulti)
        {
            try
            {
                ...
                else
                {
                    ...
                    await Notification_Show_Status("Close", "Application has no window");
                }
            }
            catch (Exception ex)
            {
        ...
        async Task ShowProcessWindow(string processName, IntPtr windowHandleTarget, bool minimizeCtrlUI, bool skipNotification, bool launchKeyboard)
        {
            try
            {
                ...
                if (windowHandleTarget == IntPtr.Zero)
                {
                    if (!skipNotification)
                    {
                        await Notification_Show_Status("Close", "Application cannot be shown");
                    }
                ...
                if (!skipNotification)
                {
                    await Notification_Show_Status("AppMiniMaxi", "Showing " + processName);
                }
                ...
                if (!windowFocused)
                {
                    await Notification_Show_Status("Close", "Failed showing application");
